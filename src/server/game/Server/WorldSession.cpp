    ...
    if (_player)
    {
		...
		else if (Map* _map = _player->FindMap()) // prevent alt+f4 abuse 
		{
			if (_player->IsInCombat())
			{
				...
				if (!aset.empty())
					if (Battleground* _bg = _player->GetBattleground())
						_bg->HandleKillPlayer(_player, *aset.begin());
			}
		}
        ...
        for (uint32 i = 0; i < addonsCount; ++i)
        {
            ...
            if (savedAddon)
            {
                if (addon.CRC != savedAddon->CRC)
                    ...
                    TC_LOG_ERROR("misc", "ADDON: %s was known, CRC is correct (0x%x)", addon.Name.c_str(), savedAddon->CRC);
            }
            else
            {
