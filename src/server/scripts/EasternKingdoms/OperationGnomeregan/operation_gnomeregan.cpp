...
public:
    ...
    struct npc_og_infantryAI : public npc_escortAI
    {
        ...
        {
            uiGCD = 2500;
            uiGrenadeTimer = urand(10000, 15000);
            uiVariation = 0;
        }

        uint32 uiGCD;
        uint32 uiGrenadeTimer;
        uint32 uiVariation;
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest)
    {
        if (quest->GetQuestId() == QUEST_OPERATION_GNOMEREGAN)
        {
            if (!bProcessing)
            {
                ...
                creature->Say(MEK_1_1, LANG_UNIVERSAL, 0);
...
public:
    ...
    struct npc_og_rlAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*who*/) override
        {
            if (Creature* mekkatorque = me->FindNearestCreature(NPC_MEKKATORQUE, 1000, true))
    ...
    struct npc_og_troggAI : public npc_escortAI
    {
        ...
        void JustDied(Unit * /*who*/) override
        {
            if (me->GetEntry() == NPC_GASHERIKK)
                if (Creature* mekkatorque = me->FindNearestCreature(NPC_MEKKATORQUE, 100, true))
