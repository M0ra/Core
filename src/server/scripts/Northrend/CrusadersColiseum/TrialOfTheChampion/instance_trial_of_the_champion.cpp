            ...
            switch (uiType)
            {
                case DATA_MOVEMENT_DONE:
                    ...
                    if (uiMovementDone == 3)
                    {
                        if (Creature* announcer = instance->GetCreature(uiAnnouncerGUID))
                            announcer->AI()->DoAction(ACTION_SET_HERALD_IN_POSITION);
                    }
                    break;
                case BOSS_GRAND_CHAMPIONS:
                    ...
                    {   
                        ...
                        case DONE:
                            ...
                            if (uiGrandChampionsDeaths >= 3 && !HasChestSpawned)
                            {
                                if (Creature* announcer = instance->GetCreature(uiAnnouncerGUID))
                                {
                                    ...
                                    announcer->AI()->SetData(DATA_RESET, 0);
                                    m_auiEncounter[0] = uiData;
                        ...
                        case NOT_STARTED:
                            uiGrandChampionsDeaths = 0;
                            for (GuidList::const_iterator itr = GrandChampionList.begin(); itr != GrandChampionList.end(); ++itr)
